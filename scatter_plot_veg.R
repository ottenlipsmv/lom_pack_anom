...
more_veg2<-mutate(more_veg, widthAvg = rowMeans(subset(more_veg, select= c(width1,width2, width3, width4, width5))))

more_veg3<-mutate(more_veg2, LenAvg = rowMeans(subset(more_veg2, select = c(length1, length2, length3, length4, length5))))
...
final_more_veg<-select(more_veg5, collection, Ratio, Stacey_coarse, Stacey_clade, LenAvg, widthAvg)
...
other_old_veg<-select(molec_only, collection, 'Aspect Ratio', Stacey_coarse, Stacey_clade, 'Leaflet lengths', 'leaflet widths')

other_old_veg<-mutate(other_old_veg, Ratio = other_old_veg$`Aspect Ratio`, LenAvg = other_old_veg$`Leaflet lengths`, widthAvg = other_old_veg$`leaflet widths`)

final_old_veg<-select(other_old_veg, -'Aspect Ratio', -'Leaflet lengths', -'leaflet widths')
